...
DockPanel::DockPanel(lv_obj_t *parent_panel) {
    ...
    lv_obj_align_to(batteryLabel, settingsButton, LV_ALIGN_OUT_LEFT_TOP, -25, 5);
    lv_obj_set_style_text_font(batteryLabel, &dock_panel_font_20, 0);

    WiFiLabel = lv_label_create(panel);
    lv_obj_align_to(WiFiLabel, batteryLabel, LV_ALIGN_OUT_LEFT_TOP, -7, 1);
...
void DockPanel::set_battery_level(int battery_level) {
    if (this->battery_charging) {
        lv_obj_align_to(batteryLabel, settingsButton, LV_ALIGN_OUT_LEFT_TOP, -6, 5);
        lv_obj_align_to(WiFiLabel, batteryLabel, LV_ALIGN_OUT_LEFT_TOP, -7, 1);
        lv_label_set_text_fmt(batteryLabel, CHARGE_SYMBOL "%d %%", battery_level);
    } else {
        lv_obj_align_to(batteryLabel, settingsButton, LV_ALIGN_OUT_LEFT_TOP, -5, 5);
        lv_obj_align_to(WiFiLabel, batteryLabel, LV_ALIGN_OUT_LEFT_TOP, -7, 1);
